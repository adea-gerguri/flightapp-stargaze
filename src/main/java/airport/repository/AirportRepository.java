...
import airport.models.dto.AirportGroupByAirlineDto;
...
public class AirportRepository {
    ...
    public Uni<UpdateResult> update(String id, Document update) {
        return MongoUtil.updateOne(getCollection(), id, update);
    }
    ...
    public Uni<List<AirportGroupByAirlineDto>> groupAirportsByAirline(){
        List<Bson> pipeline = List.of(
                Aggregates.group("$airlineName", Accumulators.sum("airportCount", 1)),
                Aggregates.project(Projections.fields(
                        Projections.computed("airlineName",  "$_id"),
                        Projections.include("airportCount")
                ))
        );
        return MongoUtil.aggregate(getCollection(), pipeline, AirportGroupByAirlineDto.class);
    }
}
